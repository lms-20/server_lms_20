...
func (h *handler) Get(c echo.Context) error {
	...
	return res.CustomSuccessBuilder(200, dto.FormatCourses(result.Datas), "Get Course Success", nil).Send(c)
}

func (h *handler) Create(c echo.Context) error {
	cc := c.(*abstraction.Context)

	payload := new(dto.CourseCreateRequest)
...
func (h *handler) Update(c echo.Context) error {
	cc := c.(*abstraction.Context)
	id := c.Param("id")
	payload := new(dto.CourseUpdateRequest)
	if err := c.Bind(&payload.CourseEntity); err != nil {
		return res.ErrorBuilder(&res.ErrorConstant.BadRequest, err).Send(c)
	}
	if err := c.Validate(payload); err != nil {
		return res.ErrorBuilder(&res.ErrorConstant.Validation, err).Send(c)
...
func (h *handler) Delete(c echo.Context) error {
	...
	message := fmt.Sprintf("success delete course id : %d", id)
